...
import { invoiceStatusLabel, ui } from "@/lib/i18n/ui";

function formatDate(iso: string) {
  try {
    return new Date(iso + "T12:00:00").toLocaleDateString(
      ui.invoices.formatLocale,
      {
        year: "numeric",
        month: "short",
        day: "numeric",
      ...
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.25em] text-[var(--accent)]">
            {ui.invoices.listKicker}
          </p>
          <h1 className="mt-2 text-2xl font-semibold text-[var(--foreground)]">
            {ui.invoices.listTitle}
          </h1>
          <p className="mt-1 max-w-lg text-sm text-[var(--muted)]">
            {ui.invoices.listIntro}
          </p>
        </div>
        <Link
        ...
        <div className="overflow-x-auto rounded-xl border border-white/10">
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead className="border-b border-white/10 bg-black/30 text-xs uppercase tracking-widest text-[var(--muted)]">
              <tr>
                <th className="px-4 py-3 font-medium">{ui.invoices.colDate}</th>
                <th className="px-4 py-3 font-medium">{ui.invoices.colRoster}</th>
                <th className="px-4 py-3 font-medium">{ui.invoices.colStatus}</th>
                <th className="px-4 py-3 font-medium">{ui.invoices.colLines}</th>
                <th className="px-4 py-3 font-medium text-right">
                  {ui.invoices.colTotal}
                </th>
              ...
              {listQuery.data?.map((inv) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-[var(--foreground)]">
                      {inv.apartment?.name ??
                        ui.invoices.profileFallback(inv.apartment_id)}
                    </td>
                    <td className="px-4 py-3">
                      <span className="rounded-md border border-white/10 px-2 py-0.5 text-xs text-[var(--muted)]">
                    ...
                    <td
                      className={`px-4 py-3 text-right font-medium ${
                        total < 0 ? "text-red-300/95" : "text-[var(--accent)]"
                      }`}
                    >
                      {total.toFixed(2)}
                    </td>
